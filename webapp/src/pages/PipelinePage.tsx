				...
				<div className="flex items-center">
					{steps.map((step, idx) => (
						<div key={step.id} className="flex items-center flex-1 last:flex-none">
							...
							{idx < steps.length - 1 && (
								<div className={`flex-1 h-px mx-0.5 ${isStepComplete(step.id) ? "bg-emerald-500/30" : "bg-white/5"}`} />
							)}
						</div>
					))}
				</div>
			...
			<div className="flex-1 overflow-y-auto p-4 space-y-4">
				{steps.map((step) => {
					...
					return (
						...
						>
							<button onClick={() => goToStep(step.id)} className="w-full flex items-center gap-3 px-4 py-3 text-left">
								<div
								...
								<div className="px-4 pb-4 space-y-3 border-t border-white/5 pt-3">
									...
									{step.id === "geometry" && (
										<>
											...
											<div className="grid grid-cols-2 gap-3">
												...
												{geometrySource === "new" ? (
													<>
														{field(
															"Domain Type",
															select(domainType, setDomainType, ["channel", "pipe", "box", "nozzle"]),
														)}
														{field(
															"Length (m)",
											...
											<div className="flex gap-2">{btn("Create Domain", handleGeometrySubmit)}</div>
										</>
									)}
											...
											<div className="flex gap-3">
												...
												>
													<div className="flex items-center gap-2 mb-2">
														...
														<span
															className={`text-sm font-medium ${solver === "openfoam" ? "text-slate-200" : "text-slate-400"}`}
														>
															OpenFOAM
														</span>
													</div>
												...
												>
													<div className="flex items-center gap-2 mb-2">
														...
														<span
															className={`text-sm font-medium ${solver === "fluidx3d" ? "text-slate-200" : "text-slate-400"}`}
														>
															FluidX3D
														</span>
													</div>
											...
											<div className="grid grid-cols-2 gap-3">
												...
												{solver === "openfoam" ? (
													...
													<>
														{field(
															"Velocity (m/s)",
															input(inletVelocity, (v) => setInletVelocity(Number(v)), { step: 0.001 }),
														)}
														{field(
															"Viscosity (m²/s)",
															input(fluidNu, (v) => setFluidNu(Number(v))),
														)}
														{field(
															"Density (kg/m³)",
															input(fluidDensity, (v) => setFluidDensity(Number(v))),
														)}
														{field(
															"Resolution X",
														...
														<div className="col-span-2 flex items-center gap-2 text-xs text-slate-500">
															<Cpu size={12} />
															Total cells: {(resX * resY * resZ).toLocaleString()} | ~
															{((resX * resY * resZ * 55) / 1e9).toFixed(2)} GB VRAM
														</div>
													</>
												)}
											</div>
											...
											{solver === "openfoam" ? (
												<>
													<p className="text-xs text-slate-500">
														Configure boundary conditions for inlet, outlet, and walls
													</p>
												...
												<div className="space-y-3">
													<p className="text-xs text-slate-500">
														FluidX3D boundary conditions are embedded in the setup file. No separate BC configuration
														needed.
													</p>
									...
									{step.id === "results" && (
										<>
											...
											<div className="flex gap-2">{btn("Read Results", handleReadResults)}</div>
																	...
																	<div key={k} className="text-center">
																		...
																		<p
																			className={`text-sm font-mono ${
																				typeof v === "number" && v < 1e-4 ? "text-green-400" : "text-amber-400"
																			}`}
																		>
