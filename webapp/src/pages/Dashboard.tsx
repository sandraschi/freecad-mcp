    ...
    <div className="space-y-6 animate-in fade-in">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div className="bg-[#0f0f12] border border-white/5 rounded-2xl p-5 space-y-3">
          ...
          <p className="text-xs text-slate-600">{status?.bridge_mode === "tcp" ? "TCP Bridge (AP214 capable)" : status?.bridge_mode === "subprocess" ? "Subprocess (limited)" : "Offline"}</p>
        </div>

        {/* OpenFOAM & Docker */}
        <div className="bg-[#0f0f12] border border-white/5 rounded-2xl p-5 space-y-3">
          <div className="flex items-center gap-2 text-emerald-400"><Waves size={18} /> OpenFOAM</div>
          {health ? (
            <>
              <div className="flex items-center gap-2 text-sm">
                {health.docker_available ? <CheckCircle2 size={16} className="text-emerald-400" /> : <XCircle size={16} className="text-red-400" />}
              ...
              {!health.openfoam_image && health.docker_available && <p className="text-xs text-amber-400/70">Run: docker pull openfoam/openfoam10-paraview56</p>}
            </>
        ...
        {/* FluidX3D */}
        <div className="bg-[#0f0f12] border border-white/5 rounded-2xl p-5 space-y-3">
          <div className="flex items-center gap-2 text-amber-400"><Zap size={18} /> FluidX3D (GPU)</div>
          {health ? (
            <>
              <div className="flex items-center gap-2 text-sm">
                {health.fluidx3d_path ? <CheckCircle2 size={16} className="text-emerald-400" /> : <XCircle size={16} className="text-red-400" />}
              ...
              {!health.fluidx3d_path && <p className="text-xs text-amber-400/70">git clone https://github.com/ProjectPhysX/FluidX3D.git</p>}
            </>
          ) : <Loader2 className="animate-spin" />}
        </div>
      ...
      <div className="bg-[#0f0f12] border border-white/5 rounded-2xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
          ...
          <a href="/chat" className="px-3 py-2 bg-white/5 hover:bg-white/10 rounded-xl text-sm text-sky-400 text-center">Ask CAD Expert</a>
          <a href="/marketplace" className="px-3 py-2 bg-white/5 hover:bg-white/10 rounded-xl text-sm text-orange-400 text-center">Marketplace</a>
          <a href="/apps" className="px-3 py-2 bg-white/5 hover:bg-white/10 rounded-xl text-sm text-slate-300 text-center">App Launcher</a>
          <a href="/help" className="px-3 py-2 bg-white/5 hover:bg-white/10 rounded-xl text-sm text-slate-500 text-center">Help</a>
        </div>
      </div>
    </div>
  );
