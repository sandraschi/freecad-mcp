import { Box, Download, FileText, Loader2, Upload } from "lucide-react";
	...
	const handleUpload = async () => {
		...
		try {
			...
			const conv = await fetch(`${API}/api/v1/control/tool`, {
				...
				body: JSON.stringify({
					tool: "step_to_stl",
					arguments: { file_name: file.name, output_name: file.name.replace(/\.(step|stp)$/i, ".stl") },
				}),
			});
		...
		<div className="max-w-2xl space-y-6">
			...
			<div className="bg-[#0f0f12] border border-white/5 rounded-2xl p-6 space-y-4">
				...
				<button
					...
					className="w-full py-3 rounded-xl bg-indigo-600 hover:bg-indigo-500 disabled:opacity-40 text-white font-bold flex items-center justify-center gap-2"
				>
				...
				{result && (
					<div className="p-4 rounded-xl bg-emerald-950/30 border border-emerald-500/20 space-y-2">
						<p className="text-emerald-400 font-bold flex items-center gap-2">
							<Download size={16} /> Conversion Complete
						</p>
						<p className="text-sm text-slate-400">
							{result.output} — {result.size_kb} KB
						</p>
						{result.objects && <p className="text-sm text-slate-400">{result.objects} objects exported</p>}
						<a
							href={`${API}/api/v1/download/${result.output}`}
							download
