	...
	return (
		<div className="space-y-4 max-w-6xl mx-auto">
			...
			<div className="flex flex-wrap items-center gap-2">
				...
				<span
					className={`text-xs font-mono px-2.5 py-1.5 rounded-lg border ${sseState === "open" ? "border-emerald-500/30 text-emerald-400" : "border-red-500/30 text-red-400"}`}
				>
					SSE {sseState}
				</span>
				<button
					onClick={() => setPaused((p) => !p)}
					className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg border border-white/10 text-xs text-slate-300 hover:bg-white/5"
				>
				...
				<button
					onClick={() => {
						...
						a.download = "freecad-mcp.log";
						a.click();
					}}
					className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg border border-emerald-500/20 text-xs text-emerald-300 hover:bg-emerald-500/10"
				>
					<Download size={12} /> Export
				</button>
				<button
					onClick={() => setLines([])}
					className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg border border-red-500/20 text-xs text-red-300 hover:bg-red-500/10"
				>
