...
export default function Fluidx3dPage() {
	...
	const connectWS = useCallback(() => {
		...
		ws.onmessage = (e) => {
			const text = e.data;
			if (text === "__SIMULATION_COMPLETE__") {
			...
			for (const line of lines) {
				...
				if (m) {
					setForceHistory((prev) => [
						...prev.slice(-500),
						{ step: parseInt(m[1]), Fx: parseFloat(m[2]), Fy: parseFloat(m[3]), Fz: parseFloat(m[4]) },
					]);
				}
	...
	const StatusBadge = ({ ok, label }: { ok: boolean; label: string }) => (
		<span
			className={`text-xs px-2 py-0.5 rounded-full font-medium ${ok ? "bg-emerald-500/20 text-emerald-400" : "bg-red-500/20 text-red-400"}`}
		>
			{ok ? "✓" : "✗"} {label}
		</span>
	);
			...
			<div className="flex-1 overflow-y-auto p-4 space-y-4">
					...
					<div className="space-y-4">
						...
						{status?.error && (
							<div className="flex items-start gap-3 p-4 bg-amber-500/10 border border-amber-500/20 rounded-xl">
								...
								<div className="text-sm text-amber-200">
									...
									<code className="text-xs bg-black/30 px-2 py-0.5 rounded block mt-2">
						...
						<div className="p-4 bg-amber-500/10 border border-amber-500/20 rounded-xl">
							...
							<p className="text-xs text-slate-400 mb-2">
								Both are available in this server. OpenFOAM runs on CPU inside Docker (GPU is idle). FluidX3D runs on
								GPU via OpenCL.
							</p>
				...
				{activeTab === "setup" && (
					<div className="space-y-4 max-w-2xl">
						...
						<div className="grid grid-cols-2 gap-3">
							...
							{field(
								"Resolution X",
								input(resX, (v) => setResX(Number(v)), { min: 16, step: 64 }),
							)}
							{field(
								"Resolution Y",
								input(resY, (v) => setResY(Number(v)), { min: 16, step: 32 }),
							)}
							{field(
								"Resolution Z",
								input(resZ, (v) => setResZ(Number(v)), { min: 16, step: 32 }),
							)}
							<div className="col-span-2 flex items-center gap-2 text-xs text-slate-500">
								<Cpu size={12} />
								Total cells: {(resX * resY * resZ).toLocaleString()} | ~{((resX * resY * resZ * 55) / 1e9).toFixed(2)}{" "}
								GB VRAM (FP32) | ~{((resX * resY * resZ * 55) / 2 / 1e9).toFixed(2)} GB (FP16)
							</div>
							{field(
								"Length (m)",
								input(lengthM, (v) => setLengthM(Number(v)), { step: 0.1 }),
							)}
							{field(
								"Velocity (m/s)",
								input(velocityMs, (v) => setVelocityMs(Number(v)), { step: 0.001 }),
							)}
							{field("Viscosity (m²/s)", input(viscosity, setViscosity))}
							{field(
								"Density (kg/m³)",
								input(density, (v) => setDensity(Number(v))),
							)}
							{field(
								"Time Steps",
								input(timeSteps, (v) => setTimeSteps(Number(v)), { step: 10000 }),
							)}
							{field(
								"Write Interval",
								input(writeInterval, (v) => setWriteInterval(Number(v)), { step: 100 }),
							)}
							...
							<div className="space-y-4">
								<div className="p-4 bg-emerald-500/10 border border-emerald-500/20 rounded-xl space-y-1">
									...
									<p className="text-xs text-slate-400">
										Re ≈ {String(result.data.Re_estimate)} | {String(result.data.resolution)} |{" "}
										{String(result.data.cells)} cells
									</p>
									<p className="text-xs text-slate-500 font-mono">{String(result.data.setup_file)}</p>
								</div>
								{!!result.data.stl_file_name && (
									<StlViewer url={`/api/v1/case-files/${caseName}/${result.data.stl_file_name}`} height={400} />
								)}
							</div>
						)}
					</div>
						...
						{result?.data && (
							<div className="p-4 bg-emerald-500/10 border border-emerald-500/20 rounded-xl space-y-1">
								...
								{!!result.data.warnings && (
									<p className="text-xs text-amber-400 mt-1">{String(result.data.warnings).slice(0, 500)}</p>
								)}
							</div>
						)}
					</div>
				...
				{activeTab === "run" && (
					<div className="space-y-4 max-w-2xl">
						...
						<div className="grid grid-cols-2 gap-3">
							...
							{field(
								"GPU Device Index",
								input(gpuDevice, (v) => setGpuDevice(Number(v)), { min: 0 }),
							)}
							{field(
								"Timeout (s)",
								input(timeoutS, (v) => setTimeoutS(Number(v)), { min: 10, step: 60 }),
							)}
						</div>
						{btn(
							"Run on GPU",
							() =>
								callTool("cfd_fluidx3d_run", {
									case_name: caseName,
								...
								{!!result.data.output && (
									<details className="p-3 bg-black/20 rounded-xl">
										...
										<pre className="text-xs text-slate-400 mt-2 max-h-60 overflow-y-auto whitespace-pre-wrap font-mono">
											{String(result.data.output).slice(-5000)}
										</pre>
									</details>
								)}
								{Array.isArray(result.data.vtk_files) && (result.data.vtk_files as string[]).length > 0 && (
									<div className="p-3 bg-white/5 rounded-xl border border-white/10">
										<p className="text-xs text-slate-500">
											VTK output files ({(result.data.vtk_files as string[]).length}):
										</p>
									</div>
								)}
							</div>
				...
				{activeTab === "live" && (
					<div className="space-y-4 max-w-4xl">
						...
						<div className="flex items-end gap-3">
							{field(
								"Case Name",
								input(caseName, (v) => setCaseName(v)),
							)}
							{field(
								"GPU Device",
								input(gpuDevice, (v) => setGpuDevice(Number(v)), { min: 0 }),
							...
							{forceHistory.length > 0 && (
								<div className="bg-white/5 rounded-xl p-4 border border-white/10" style={{ height: 250 }}>
									...
									<ResponsiveContainer width="100%" height="85%">
										<LineChart
											data={forceHistory.map((f: { step: number; Fx: number; Fy: number; Fz: number }, i: number) => ({
												...f,
												idx: i,
											}))}
										>
						...
						<div className="bg-black/40 rounded-xl p-4 border border-white/5">
							...
							<pre className="text-xs font-mono text-slate-400 max-h-60 overflow-y-auto whitespace-pre-wrap">
								{liveLog}
							</pre>
						</div>
					</div>
				...
				{activeTab === "results" && (
					<div className="space-y-4 max-w-2xl">
						...
						{btn(
							"Parse Results",
							() => callTool("cfd_fluidx3d_results", { case_name: caseName }),
							<BarChart3 size={14} />,
						)}
						{result?.data && (
							<div className="space-y-3">
								...
								{Array.isArray(result.data.forces) &&
									(result.data.forces as Array<{ step: number; Fx: number; Fy: number; Fz: number }>).length > 0 && (
										<div className="space-y-1">
											<p className="text-xs text-slate-500">
												Force history (last {(result.data.forces as unknown[]).length} points)
											</p>
											<div className="max-h-40 overflow-y-auto space-y-0.5">
												{(result.data.forces as Array<{ step: number; Fx: number; Fy: number; Fz: number }>)
													.slice(-20)
													.map((f: { step: number; Fx: number; Fy: number; Fz: number }) => (
				...
				{activeTab === "video" && (
					<div className="space-y-4 max-w-3xl">
						...
						{field(
							"FPS",
							input(fps, (v) => setFps(Number(v)), { min: 1, max: 30, step: 1 }),
						)}
						{btn(
							"Render Video",
							() => callTool("cfd_fluidx3d_render", { case_name: caseName, fps: Number(fps) }),
							<Film size={14} />,
						)}
						...
						{result?.data && (
							<div className="space-y-4">
								{!!result.data.video_path && (
									...
									>
										<source
											src={`/api/v1/case-files/${encodeURIComponent(caseName)}/${encodeURIComponent(caseName)}_simulation.webm`}
											type="video/webm"
										/>
									</video>
								)}
