import { RefreshCw, Settings as SettingsIcon, Cpu, Globe, Key, CheckCircle2, XCircle, Loader2, AlertTriangle } from "lucide-react";
...
export default function SettingsPage() {
  ...
  const discoverProviders = async () => {
    ...
    try {
      ...
      const activeProvider = savedProvider && detected.find((p) => p.name === savedProvider) ? savedProvider : (detected[0]?.name || "ollama");
    ...
    <div className="max-w-2xl space-y-6" data-testid="settings-page">
      ...
      <div className="bg-[#0f0f12] border border-white/5 rounded-2xl p-6 space-y-4" data-testid="llm-section">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2"><Cpu size={16} className="text-indigo-400" /><h3 className="text-sm font-bold text-slate-400 uppercase tracking-wider">LLM Provider</h3></div>
          <button onClick={discoverProviders} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs text-slate-500 hover:text-slate-300 hover:bg-white/5 transition-all">
        ...
        <div className="grid grid-cols-3 gap-2">
          {[
            ...
            <div key={p.id} className={`flex items-center gap-2 p-3 rounded-xl border text-xs ${discover?.status[p.id] === "detected" ? "bg-emerald-500/10 border-emerald-500/20" : "bg-white/5 border-white/10"}`}>
        ...
        {!detectedProviders.length && !probing && (
          <div className="flex items-start gap-3 p-3 bg-amber-500/10 border border-amber-500/20 rounded-xl">
            ...
            <p className="text-xs text-amber-200">Install Ollama or LM Studio to enable local AI features. <a href="https://ollama.com" target="_blank" rel="noopener noreferrer" className="text-indigo-400 hover:underline">ollama.com</a></p>
          </div>
        )}
        ...
        <div>
          ...
          <select value={provider} onChange={(e) => onProviderChange(e.target.value)} data-testid="llm-provider-select" className="w-full bg-zinc-800 text-zinc-100 border border-zinc-600 rounded-xl px-4 py-2.5 text-sm outline-none focus:border-indigo-500/30">
            {detectedProviders.length ? detectedProviders.map((p) => (
              <option key={p.name} value={p.name}>{p.name.replace("_", " ").replace(/\b\w/g, (c) => c.toUpperCase())}</option>
        ...
        <div>
          ...
          <select value={model} onChange={(e) => onModelChange(e.target.value)} data-testid="llm-model-select" className="w-full bg-zinc-800 text-zinc-100 border border-zinc-600 rounded-xl px-4 py-2.5 text-sm outline-none focus:border-indigo-500/30">
            {availableModels.length ? availableModels.map((m) => (
