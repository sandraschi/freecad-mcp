		...
		try {
			...
			if (j.success) {
				setRecentUploads((prev) =>
					[{ name: j.filename, title: file.name.replace(/\.[^.]+$/, "") }, ...prev].slice(0, 5),
				);
			} else {
		...
		try {
			const ext =
				item.source === "thingiverse"
					? ".zip"
					: item.download_url.toLowerCase().includes(".step")
						? ".STEP"
						: item.download_url.toLowerCase().includes(".stp")
							? ".stp"
							: ".stl";
			...
			if (j.success) {
				...
				alert(
					`Downloaded: ${j.filename} (${(j.size_bytes / 1024).toFixed(1)} KB)${extracted ? `\nExtracted: ${extracted}` : ""}`,
				);
			} else {
					...
					<p className="text-slate-500 text-sm">
						...
						{category && (
							<span className="text-slate-600">
								{" "}
								in <span className="text-indigo-400">{categories.find((c) => c.id === category)?.label}</span>
							</span>
						)}
					</p>
					...
					{uploading ? (
						...
						<div className="space-y-2">
							...
							<p className="text-sm text-slate-400">
								Downloaded from the marketplace? <strong className="text-indigo-400">Drop your file here</strong> or
								click to select
							</p>
			...
			{recentUploads.length > 0 && (
				<div className="bg-[#0f0f12] border border-white/5 rounded-2xl p-4 space-y-2">
					<h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider flex items-center gap-2">
						<CheckCircle size={12} className="text-emerald-400" /> Recently imported to depot
					</h3>
					{recentUploads.map((u) => (
						<a
							key={u.name}
							href={`/api/v1/download/${u.name}`}
							target="_blank"
...
function CardResult({
	...
	onDownload: (item: ModelResult) => void;
}) {
	return (
			...
			<div className="p-4 space-y-2">
				...
				<div className="flex items-center justify-between pt-2 border-t border-white/5">
					<div className="flex items-center gap-3 text-xs text-slate-500">
						...
						<span className="flex items-center gap-1">
							<Star size={12} /> {item.likes.toLocaleString()}
						</span>
					</div>
					<button
