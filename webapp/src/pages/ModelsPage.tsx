...
export default function ModelsPage() {
	...
	return (
		<div className="max-w-5xl space-y-6">
			...
			{selected && isStl(selected) && <StlViewer url={`/api/v1/download/${selected}`} height={350} />}
			...
			{info && (
				<div className="bg-[#0f0f12] border border-white/5 rounded-2xl p-4 space-y-2">
					...
					<pre className="text-xs text-slate-300 font-mono whitespace-pre-wrap">
						{JSON.stringify(info.data || info, null, 2)}
					</pre>
				</div>
			)}
		</div>
	...
	return (
		<div className="bg-[#0f0f12] border border-white/5 rounded-2xl p-4 space-y-2">
			...
			{loading ? (
				...
				<p className="text-slate-600 text-sm">No files</p>
			) : (
				files.map((f) => (
					<div
						key={f.name}
						onClick={() => onSelect(f.name)}
						className={`flex items-center justify-between p-3 rounded-xl cursor-pointer text-sm ${selected === f.name ? "bg-indigo-500/10 border border-indigo-500/30" : "bg-white/5 hover:bg-white/10"}`}
					>
						<span className="flex items-center gap-2 truncate">
							<Icon size={14} className={iconColor} /> {f.name}
						</span>
