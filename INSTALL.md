...
git clone https://github.com/sandraschi/freecad-mcp
cd freecad-mcp
just
```
...
4. Start the server:
   ...
   uv run python -m freecad_mcp.server

   # HTTP mode (for web dashboard)
   uv run uvicorn freecad_mcp.server:app --port 10944
   ```
...
5. Open `http://localhost:10944` or the frontend URL.
...
| Something else | [Open a GitHub issue](https://github.com/sandraschi/freecad-mcp/issues) |
